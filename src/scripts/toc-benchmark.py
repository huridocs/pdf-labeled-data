...
def set_labels(pdf_features: PdfFeatures, pdf_name):
    if FAST:
        prediction_path = Path(ROOT_PATH, "downloads", pdf_name + ".json")
    else:
        prediction_path = Path(ROOT_PATH, "downloads", "vgt" + pdf_name + ".json")

    content = prediction_path.read_text()
...
def get_benchmark():
    ...
    truth = [1 if x == 5 else 0 for x in truth]
    prediction = [1 if x == 5 else 0 for x in prediction]

    print("f1")
    print(f1_score(truth, prediction))

    print("precision")
    print(precision_score(truth, prediction))

    print("recall")
    print(recall_score(truth, prediction))
...
if __name__ == '__main__':
    # print(precision_score([1,1,1], [0,0,1]))
    get_benchmark()
