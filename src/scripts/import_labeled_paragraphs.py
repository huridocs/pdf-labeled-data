...
from config import XML_NAME, LABELS_FILE_NAME, LABELED_XML_DESTINATION, LABELED_DATA_DESTINATION
...
def import_structure():
    for dataset_type_name, xml_name in loop_token_type_pdfs():
        os.makedirs(join(LABELED_DATA_DESTINATION, "paragraph_extraction", dataset_type_name, xml_name))
...
def xml_labels_to_json_labels(dataset_type_name: str, xml_name: str):
    ...
    labels_path: str = join(
        LABELED_DATA_DESTINATION, "paragraph_extraction", dataset_type_name, get_folder_name(xml_name), LABELS_FILE_NAME
    )
