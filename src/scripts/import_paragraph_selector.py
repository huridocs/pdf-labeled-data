...
def import_pdfs():
    for dataset_type_name, pdf_name in loop_pdfs():
        print(dataset_type_name, pdf_name)
...
def get_labels_per_page(annotations, dataset_type_name, pdf_name):
    ...
    for annotation in annotations["annotations"]:
        ...
        bounding_box = Rectangle(
            left=int(annotation["bounds"]["left"] * 0.75),
            top=int(annotation["bounds"]["top"] * 0.75),
            right=int(annotation["bounds"]["right"] * 0.75),
            bottom=int(annotation["bounds"]["bottom"] * 0.75),
        )
