...
import { Annotation, TokenId } from './AnnotationStore';
...
export function doOverlap(a: Bounds, b: Bounds): boolean {
    const threshold = 3;
    if (a.left + threshold >= b.right || a.right <= b.left + threshold) {
        return false;
    } else if (a.bottom <= b.top + threshold || a.top + threshold >= b.bottom) {
        return false;
    }
    return true;
}
...
export function getNewAnnotation(
    page: PDFPageInfo,
    selection: Bounds,
    activeLabel: Label = { name: '', color: '#70DDBA' }
): Optional<Annotation> {
